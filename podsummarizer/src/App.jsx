...
const G = css`
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dropzone::after {
    ...
    background: radial-gradient(ellipse at 50% 0%, rgba(37,99,235,.04) 0%, transparent 60%);
    pointer-events: none;
  }
  ...
  .branch-stats { display: flex; align-items: center; gap: 16px; }
...
function exportXLSX(branches, sourceFileName) {
  ...
  // ── helpers ──
  const cell = (v, t) => ({ v, t: t || (typeof v === "number" ? "n" : "s") });
  const sc = (ws, addr, v, s) => {
    ws[addr] = { ...(typeof v === "object" && v !== null && "v" in v ? v : { v, t: typeof v === "number" ? "n" : "s" }), s };
  };

  // colour palette — one per branch
  const BRANCH_COLORS = [
  ...
  const makeStyle = (overrides) => ({ ...overrides });

  const headerStyle = (bg, fg) => makeStyle({
    font: { name: "Arial", bold: true, color: { rgb: fg || "FFFFFF" }, sz: 10 },
  ...
  const dataStyle = (bg, bold, align, color) => makeStyle({
    font: { name: "Arial", sz: 10, bold: !!bold, color: { rgb: color || "1E293B" } },
  ...
  const COL_COUNT = 5; // A=No, B=Rider, C=HitCount, D=Dates, E=Notes

  // Helper: write a full-width merged banner row
  const bannerRow = (text, bg, fg, sz, height) => {
    sc(ws, `A${r + 1}`, text, makeStyle({
      font: { name: "Arial", bold: true, sz: sz || 12, color: { rgb: fg || "FFFFFF" } },
      fill: { patternType: "solid", fgColor: { rgb: bg || "1E3A5F" } },
      alignment: { horizontal: "center", vertical: "center", wrapText: false },
    }));
  ...
  const emptyRow = (bg) => {
    for (let c = 0; c < COL_COUNT; c++) {
      ...
      sc(ws, addr, "", makeStyle({
        fill: { patternType: "solid", fgColor: { rgb: bg || "F8FAFC" } },
      }));
    }
    merges.push({ s: { r, c: 0 }, e: { r, c: COL_COUNT - 1 } });
    ws[`!rows`] = ws[`!rows`] || [];
  ...
  bannerRow(`Generated: ${dateStr}   ·   Total Branches: ${Object.keys(branches).length}   ·   Total Riders: ${allRiders.length}   ·   Total Hits: ${totalHits}`, "2C4A6E", "BFD4F0", 9, 18);
  ...
  const sumCols = ["Branch", "Riders at 100%", "Total 100% Hits", "Top Performer", "Top Performer Hits"];
  sumCols.forEach((h, i) => {
    const addr = XLSX.utils.encode_cell({ r, c: i });
    sc(ws, addr, h, sumHdrStyle);
  });
  ws[`!rows`] = ws[`!rows`] || [];
  ws[`!rows`][r] = { hpt: 20 };
  r++;
  ...
  branchEntries.forEach(([branch, riders], bi) => {
    const pal = BRANCH_COLORS[bi % BRANCH_COLORS.length];
    const bHits = riders.reduce((s, rr) => s + rr.dates.length, 0);
    const top = riders[0];
    const isEven = bi % 2 === 0;
    const rowBg = isEven ? "FFFFFF" : "F8FAFC";
    const cells = [branch, riders.length, bHits, top?.rider || "", top?.dates.length || 0];
    const aligns = ["left", "center", "center", "left", "center"];
    cells.forEach((v, i) => {
      const addr = XLSX.utils.encode_cell({ r, c: i });
      const isBranch = i === 0;
      sc(ws, addr, v, dataStyle(isEven ? "FFFFFF" : "F8FAFC", isBranch, aligns[i], isBranch ? pal.header : "1E293B"));
    });
    ws[`!rows`][r] = { hpt: 18 };
    r++;
  });
  ...
  // Grand total row
  const gtStyle = makeStyle({
    font: { name: "Arial", bold: true, sz: 10, color: { rgb: "1E293B" } },
    fill: { patternType: "solid", fgColor: { rgb: "E2E8F0" } },
    alignment: { horizontal: "center", vertical: "center" },
    border: { top: { style: "medium", color: { rgb: "94A3B8" } }, bottom: { style: "medium", color: { rgb: "94A3B8" } } },
  });
  ["GRAND TOTAL", branchEntries.length, totalHits, `${allRiders.length} riders total`, ""].forEach((v, i) => {
  ...
  branchEntries.forEach(([branch, riders], bi) => {
    ...
    // Branch title bar
    sc(ws, `A${r + 1}`, `  ${branch.toUpperCase()}`, makeStyle({
      font: { name: "Arial", bold: true, sz: 11, color: { rgb: pal.headerFg } },
      fill: { patternType: "solid", fgColor: { rgb: pal.header } },
      alignment: { horizontal: "left", vertical: "center" },
    }));
    merges.push({ s: { r, c: 0 }, e: { r, c: 2 } });

    const branchMeta = `${riders.length} rider${riders.length !== 1 ? "s" : ""}   ·   ${bHits} total hits`;
    sc(ws, XLSX.utils.encode_cell({ r, c: 3 }), branchMeta, makeStyle({
      font: { name: "Arial", sz: 9, color: { rgb: pal.headerFg }, bold: false },
      fill: { patternType: "solid", fgColor: { rgb: pal.header } },
    ...
    // Column headers
    ["#", "Rider Name", "Times Hit 100%", "Dates Achieved (100% POD Rate)", "Aliases / Notes"].forEach((h, i) => {
    ...
    riders.forEach((rider, ri) => {
      ...
      sc(ws, XLSX.utils.encode_cell({ r, c: 2 }), rider.dates.length, makeStyle({
        font: { name: "Arial", bold: true, sz: 11, color: { rgb: pal.dot } },
        fill: { patternType: "solid", fgColor: { rgb: bg } },
        alignment: { horizontal: "center", vertical: "center" },
      ...
      sc(ws, XLSX.utils.encode_cell({ r, c: 3 }), rider.dates.join("  ·  "), dataStyle(bg, false, "left", "374151"));
      sc(ws, XLSX.utils.encode_cell({ r, c: 4 }), rider.aliases?.length > 0 ? `Also: ${rider.aliases.join(", ")}` : "", dataStyle(bg, false, "left", "7C3AED"));
      ws[`!rows`][r] = { hpt: rider.dates.length > 6 ? 30 : 18 };
      r++;
    });

    // Branch subtotal row
    ["", `Branch Total  (${branch})`, bHits, "", ""].forEach((v, i) => {
      sc(ws, XLSX.utils.encode_cell({ r, c: i }), v, makeStyle({
        font: { name: "Arial", bold: true, sz: 10, color: { rgb: i === 2 ? pal.dot : "475569" } },
        fill: { patternType: "solid", fgColor: { rgb: pal.subtotal } },
        alignment: { horizontal: i === 2 ? "center" : "left", vertical: "center" },
        border: { top: { style: "thin", color: { rgb: pal.dot } }, bottom: { style: "thin", color: { rgb: pal.dot } } },
      }));
    });
    ws[`!rows`][r] = { hpt: 18 };
    r++;
  ...
  // ── FOOTER ──
  emptyRow("F8FAFC");
  bannerRow(`This report was auto-generated · Only riders with 100% POD Rate are included · ${dateStr}`, "F1F5F9", "94A3B8", 8, 16);
...
const IconTrophy = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="8 2 8 8 12 11 16 8 16 2"/><path d="M8 2H4v6c0 3.31 2.69 6 6 6h0v4"/><path d="M16 2h4v6c0 3.31-2.69 6-6 6h0v4"/><rect x="8" y="20" width="8" height="2" rx="1"/>
  </svg>
);
const IconFile = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
// ─── RESULTS PANEL (shared between current upload & history view) ─────────────
        ...
        {[
          ...
          <div key={s.label} className="stat-card">
            ...
            <div className="stat-info">
              <div className="stat-num" style={{ color: s.numColor }}>{s.value}</div>
                  ...
                  <tbody>
                    {riders.map((r, ri) => (
                      <tr key={r.rider + ri}>
                        ...
                        <td>
                          <div className="rider-name">{r.rider}</div>
                          {r.aliases?.length > 0 && <div className="rider-alias">Also known as: {r.aliases.join(", ")}</div>}
                        </td>
                        <td style={{ textAlign: "center" }}>
                          <span className="hit-badge" style={{ background: pal.hit.bg, color: pal.hit.color }}>
                            <IconTrophy />{r.dates.length}×
                          </span>
                        </td>
                        <td>
...
export default function App() {
  ...
  const [history, setHistory]         = useState([]);       // [{id,fileName,uploadedAt,branches}]
  const [activeTab, setActiveTab]     = useState("upload"); // "upload" | "history"
  const [historyView, setHistoryView] = useState(null);     // history entry being viewed
  ...
  const processFile = useCallback((file) => {
    ...
    setBranches(null); setFileName(file.name); setHistoryView(null); setActiveTab("upload");
      ...
      try {
        ...
        if (!Object.keys(result).length) {
          ...
          setHistory(prev => {
            ...
            (async () => {
              try { await window.storage.set(HISTORY_KEY, JSON.stringify(updated)); } catch (_) {}
            })();
            return updated;
          });
        }
  ...
  // Active data = either current upload or history view
  ...
  const allRiders   = viewData ? Object.values(viewData.branches).flat() : [];
  const totalHits   = allRiders.reduce((s, r) => s + r.dates.length, 0);
        ...
        <div className="layout">
            ...
            <div className="sidebar-list">
              ...
              {storageReady && history.length === 0 && (
                <div className="sidebar-empty">
                  ...
                  No uploads yet.<br />Files you process will appear here so you can re-export them anytime.
                </div>
              )}
              {storageReady && history.map(entry => {
                ...
                return (
                  <div
                    key={entry.id}
                    className={`hist-item${isActive ? " active" : ""}${isDeleting ? " deleting" : ""}`}
                    onClick={() => { setHistoryView(isActive ? null : entry); }}
                  >
                    <div className="hist-item-name" title={entry.fileName}>
                      📄 {entry.fileName}
                    </div>
                    ...
                    <div className="hist-item-meta" style={{ marginTop: 3, opacity: .7 }}>
          ...
          <div className="content-area">
            <main className="main">
                  ...
                  >
                    ...
                    <div className="dz-inner">
                      ...
                      <div className="dz-title">
                        {loading ? "Processing your file…" : drag ? "Release to upload" : "Drop Excel file here or click to browse"}
                      </div>
              ...
              {/* ── ALERTS (only for current upload) ── */}
              {!historyView && error && (
                <div className="alert err">
                  <div className="alert-icon"><IconWarn /></div>
                  <div><div className="alert-title">Unable to Process File</div><div>{error}</div></div>
                </div>
              )}
              {!historyView && warnings.length > 0 && (
                <div className="alert warn">
                  <div className="alert-icon"><IconWarn /></div>
                  <div>
                    <div className="alert-title">Processing Notices ({warnings.length})</div>
                    <div className="alert-list">{warnings.map((w, i) => <div key={i}>• {w}</div>)}</div>
                  </div>
                </div>
              )}
              {!historyView && mergeEvents.length > 0 && (
                <div className="alert merge">
                  <div className="alert-icon"><IconLink /></div>
                  <div>
                    <div className="alert-title">Name Merges Applied ({mergeEvents.length})</div>
                    <div className="alert-list">
                      {mergeEvents.map((e, i) => (
                        <div key={i}><strong>[{e.branch}]</strong> {e.canonical} ← {e.aliases.join(", ")}</div>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="empty-card">
                  ...
                  <div className="how-grid">
                    {[
                      ...
                      { icon: "⬇", title: "Export Report",    desc: "Downloads a polished single-sheet Excel report, beautifully formatted by branch." },
                    ].map(item => (
                  ...
                  <div className="col-detect">
                    ...
                    <span className="col-kw">Branch</span><span className="col-kw">Hub</span><span className="col-kw">Area</span><span className="col-kw">Zone</span><span className="col-kw">Region</span><span className="col-kw">Depot</span>
                    <span style={{ color: "var(--text3)", margin: "0 8px" }}>·</span>
                    <strong>Rider:</strong>
                    <span className="col-kw">Rider</span><span className="col-kw">Courier</span><span className="col-kw">Driver</span><span className="col-kw">Name</span>
                    <span style={{ color: "var(--text3)", margin: "0 8px" }}>·</span>
                    <strong>Rate:</strong>
                    <span className="col-kw">POD Rate</span><span className="col-kw">POD%</span><span className="col-kw">Delivery Rate</span>
                    <span style={{ color: "var(--text3)", margin: "0 8px" }}>·</span>
                    <strong>Date:</strong>
                    <span className="col-kw">Date</span><span className="col-kw">Day</span><span className="col-kw">Period</span>
